 ...
 */
class ModuleList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        parent::setActiveRecord('Module'); // defines the active record
        parent::setFilterField('name'); // defines the filter field
        parent::setDefaultOrder('id', 'desc');

        // creates the form
        $this->form = new BootstrapFormWrapper(new TQuickForm('form_search_module'));

        // create the form fields
        $filter = new TEntry('name');
        $filter->setValue(TSession::getValue('module_name'));
        ...
        $this->form->addQuickAction(_t('New'), new TAction(array('ModuleForm', 'onEdit')), 'fa:plus-circle green');
        ...
        $this->datagrid->addQuickColumn(_t('Title'), 'title', 'left', '20%');
        $this->datagrid->addQuickColumn(_t('Name'), 'name', 'left', '20%');
        $this->datagrid->addQuickColumn('Description', 'description', 'left', '30%');
        $this->datagrid->addQuickColumn('Product', 'product_name', 'left', '40%');

        // add the actions to the datagrid
        $this->datagrid->addQuickAction(_t('Edit'), new TDataGridAction(array('ModuleForm', 'onEdit')), 'id', 'fa:edit blue');
        ...
        $panelForm = new TPanelGroup(('Listadem de Módulos'));
